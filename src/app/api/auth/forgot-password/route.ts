...
export async function POST(req: NextRequest) {
  ...
  const genericOk = NextResponse.json({
    success: true,
    message: 'إذا كان الرقم مسجلاً، هنبعتلك رابط إعادة التعيين على واتساب.',
  });
  ...
  // Primary channel: WhatsApp to the user's phone (every account has one).
  try {
  ...
  // Fallback channel: email, only if the user provided a real one.
